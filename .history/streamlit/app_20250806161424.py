...
with tabs[2]:
    ...
    # 🔘 Add button to delete all files
    if st.button("🗑️ Clear All Files"):
        confirm = st.warning("Are you sure you want to delete all uploaded files? This cannot be undone.")
        if st.button("✅ Yes, delete all files"):
            del_res = requests.delete(f"{API_BASE_URL}/clear-files")
            if del_res.status_code == 200:
                st.success("✅ All files have been deleted.")
            else:
                st.error("❌ Failed to delete files from the server.")
