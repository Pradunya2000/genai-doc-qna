...
with tabs[0]:
    ...
    for slot_id in list(st.session_state.file_slots):
        col_uploader, col_slot_remove = st.columns([8, 1])
        ...
        # Small ✕ for removing empty slot
        with col_slot_remove:
            if st.button("✕", key=f"remove_slot_{slot_id}", help="Remove this file slot"):
                remove_file_slot(slot_id)

        # If file uploaded, store and show small ✕ to remove uploaded file
        if file:
            st.session_state.uploaded_files[slot_id] = file
            col_file, col_file_remove = st.columns([8, 1])
            with col_file:
                st.write(f"📄 {file.name}")
            with col_file_remove:
                if st.button("✕", key=f"remove_uploaded_{slot_id}", help="Remove this file"):
                    remove_file_slot(slot_id)
