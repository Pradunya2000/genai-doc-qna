    ...
    if st.button("📤 Process All Files", type="primary"):
        if not all_uploaded_files:
            ...
            with st.spinner("🚀 Uploading and processing documents..."):
                filenames = save_uploaded_files(all_uploaded_files)
                process_documents()
                st.success(f"✅ Files uploaded and processed successfully: {filenames}")
            st.experimental_rerun()
...
with tab2:
    ...
    file_history = list_uploaded_files()
    if not file_history:
    ...
    if file_history:
        ...
        if st.button("🗑️ Clear All Data", type="secondary"):
            with st.spinner("🗑️ Clearing all files and embeddings..."):
                clear_all_data()
                st.session_state.questions = [""]
                st.success("✅ All files and embeddings cleared!")
            st.experimental_rerun()
    else:
