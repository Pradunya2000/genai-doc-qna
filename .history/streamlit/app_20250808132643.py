...
with tabs[0]:
    ...
    # --- Render each file slot with a small inline '✕' to remove the slot ---
    for slot_id in st.session_state["file_slots"]:
        # wide uploader column + tiny column for the inline remove icon
        col_uploader, col_remove_slot = st.columns([9, 1])

        # file uploader sits in the wide column
        with col_uploader:
        ...
        # small inline '✕' for removing this empty slot (or slot with selected file)
        with col_remove_slot:
            # This button is intentionally small because the column is narrow.
            if st.button("✕", key=f"remove_slot_{slot_id}", help="Remove this file slot"):
                remove_slot(slot_id)
                # remove_slot calls st.rerun() already

        # If a file was selected, show a compact row with filename and a small ✕ to remove that file
        if file_obj:
            # save uploaded file in session state
            st.session_state["uploaded_files"][slot_id] = file_obj

            # Show the filename with a tiny remove control to remove only the file (keeps the slot)
            ...
            with col_remove_file:
                if st.button("✕", key=f"remove_file_{slot_id}", help="Remove this uploaded file"):
                    # remove the uploaded file but keep the slot available
                    st.session_state["uploaded_files"].pop(slot_id, None)
                    # also clear the uploader widget state by rerunning
                    st.rerun()

    # --- Add slot button (adds one new uploader slot) ---
    if st.button("➕ Add Another File"):
        add_file_slot()

    # --- Preview the collected files to upload (skip None slots) ---
