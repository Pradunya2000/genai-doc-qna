...
with tabs[2]:
    ...
    if res.status_code == 200:
        ...
        if not files:
            ...
            # Show Delete All button inside an expander to avoid accidental clicks
            with st.expander("⚠️ Danger Zone: Delete All Uploaded Files"):
                st.warning("Are you sure you want to delete **all uploaded files and embeddings**?")
                if st.button("🗑️ Delete All Files and Embeddings"):
                    delete_res = requests.delete(f"{API_BASE_URL}/clear-files/")
                    if delete_res.status_code == 200:
                        st.success("✅ All files and embeddings deleted.")
                    else:
